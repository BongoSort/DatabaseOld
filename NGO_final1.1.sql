CREATE SCHEMA IF NOT EXISTS `NGOs4`;
...
CREATE TABLE `Supporter`(
    ...
    `city`		VARCHAR(20),
    `birthday`	DATE			NOT NULL 	CHECK(`birthday` BETWEEN DATE '1922-01-01' AND '2004-01-01')
);
...
-- inserts testers for 4 donations to all ngos --
-- INSERT INTO Donations VALUES ('Have a coin', 1, '2022-02-18', 'vigga@yahoo.dk', 'Danish Refugee Council');
-- INSERT INTO Donations VALUES ('Have a coin', 1, '2022-02-18', 'vigga@yahoo.dk', 'KDG');
-- INSERT INTO Donations VALUES ('Have a coin', 1, '2022-02-18', 'vigga@yahoo.dk', 'Amnesty International');
-- INSERT INTO Donations VALUES ('Have a coin', 1, '2022-02-18', 'vigga@yahoo.dk', 'SOS Childrens Villages');

-- Updates level in Supports --
UPDATE Supports s SET `level` = 
	(SELECT COUNT(*) FROM Donations d 
	WHERE d.`email` = s.`email` 
	AND d.`ngo_name` = s.`ngo_name`);

    -- Create trigger, so level get updated correctly --
CREATE TRIGGER UpdateLevel
    ...
    -- Create an index to find Supports ngo_name faster --
CREATE INDEX sup_ngo ON Supports(ngo_name);

-- Creating the a view containing all info about KGD supporters -- 
CREATE OR REPLACE VIEW KDG_sup AS 
	SELECT r.* FROM Supporter r, Supports s 
		WHERE r.`email` = 
			(SELECT s.`email` WHERE s.`ngo_name` = 'KDG');

	-- grant SELECT privilege to user, and grant grant option to user --
GRANT SELECT ON KDG_sup TO user
WITH GRANT OPTION;

-- Adding citizenship to Supportertable, where default is Danish--
ALTER TABLE Supporter ADD `citizenship` VARCHAR(20) DEFAULT 'Danish';
   
-- Finding the supporters who have donated to all ngos --
SELECT * FROM supporter WHERE `email` = 
	(SELECT `email` FROM 
		(SELECT `email` FROM supports WHERE `level` > 0) AS emailtable 
			GROUP BY `email` HAVING count(*) = (SELECT count(`name`) FROM ngo));
 

-- Tests --
-- SELECT * FROM kdg_sup;
-- SELECT Count(`name`) FROM ngo;
-- SELECT * FROM supports WHERE `email`= 'vigga@yahoo.dk';
-- SELECT `email` FROM supports WHERE level > 0;